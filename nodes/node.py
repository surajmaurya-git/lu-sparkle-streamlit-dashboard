...
def draw_unit_1_dashboard():
    ...
    values={"water_limit":None,"expiry":None,"water_consumption":None, "left_water_limit": None, "tds_1":None, "tds_2": None,"PumpRunTime":None,"flowRate":None} 
    ...
    # fetch pump run time time
    wRes = node.get_valueStore(key="PumpRunTime")
    if wRes.get("isSuccess") is True and wRes.get("value") is not None:
         values["PumpRunTime"] = float(wRes.get("value"))
    # fetch water consumption
    wRes = node.get_valueStore(key="flowRate")
    if wRes.get("isSuccess") is True and wRes.get("value") is not None:
         values["flowRate"] = float(wRes.get("value"))
